...
class DivisionController extends GetxController {
  ...
  var onTaped = false.obs;
  var isButtonClickable = true.obs;

  void buttonClicked() async {
    Duration time = Duration(seconds: 5);

    isButtonClickable.value = false; //make the button disable to making variable false.
    print("Clicked Once");
    Future.delayed(time, () {
      isButtonClickable.value = true; //future delayed to change the button back to clickable
    });
  }
